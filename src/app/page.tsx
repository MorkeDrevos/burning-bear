...
export default function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (s.burnIntervalMs == null)    s.burnIntervalMs = burnMins * 60_000;
  ...
  useEffect(() => {
    setData((prev) => {
      ...
      const toMsLocal = (v?: number) =>
        typeof v === 'number' ? (v >= 10_000 ? v : v * 60_000) : undefined;

      const burnI = s.burnIntervalMs ?? toMsLocal(s.burnIntervalMinutes);
      const buyI  = s.buybackIntervalMs ?? toMsLocal(s.buybackIntervalMinutes);
  ...
  style={{
    top: broadcast.on
      ? 'calc(var(--safe-top, 0px) + 10px)'
      : 'calc(var(--safe-top, 0px) + 84px)',
  }}
>
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 md:py-5">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-sm font-semibold text-white/80 underline decoration-white/20 underline-offset-[6px] hover:text-amber-100 hover:decoration-amber-300/50 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-amber-300/25 rounded-sm"
            >
...
/>
      ...
      <section className="bg-[#0d1411] border-t border-white/5 pt-8 pb-5">
        <div className="mx-auto flex flex-wrap items-center justify-center gap-5 text-[16px] md:text[17px] text-white/90 font-medium px-4 max-w-6xl">
          ...
          <div className="inline-flex items-center gap-2">
            ...
            <code className="font-mono text-[15px] text-white/70" title={FULL_TOKEN_ADDRESS}>
      ...
      {/* ===== Live Burn Log ===== */}
      <section id="log" className="w-full px-4 sm:px-6 lg:px-8 mt-6 scroll-mt-24 md:scroll-mt-28">
      ...
      {/* ===== How It Works ===== */}
      <section id="how" className="mx-auto max-w-6xl px-4 pt-12 pb-16 md:pt-14 md:pb-20 scroll-mt-[88px] md:scroll-mt-[100px] lg:scroll-mt-[116px]">
      ...
      <section id="bear" className="relative w-full overflow-hidden py-20 md:py-24">
        ...
        <div className="mx-auto max-w-6xl px-4">
          <div className="mx-auto mb-8 w-[260px] md:w-[320px] relative">
            <img src="/img/burning_bear_BBURN_v2.gif" alt="The Burning Bear" className="w-full h-auto rounded-2xl shadow-ember" />
            <div className="pointer-events-none absolute inset-0 rounded-2xl bg-amber-500/10 blur-[60px]" />
          </div>

          <h2 className="text-4xl md:text-5xl font-extrabold text-amber-300 text-center">Meet The Burning Bear</h2>
          ...
          <div className="mt-16 w-full px-0">
            <h3 className="text-2xl md:text-3xl font-bold text-amber-300 text-center mb-8">Why The Burning Bear Exists</h3>
      ...
      {/* ===== Roadmap ===== */}
      <section id="roadmap" className="py-16 md:py-24 scroll-mt-[88px] md:scroll-mt-[100px] lg:scroll-mt-[116px]">
          ...
          <div className="mt-10 space-y-10">
            {[
              ...
              },
              {
                ...
                outcome: "🔥 Outcome: From meme → movement — the culture of burning & proof of supply.",
              },
              {
                icon: "🐻",
                ...
                outcome: "🔥 Outcome: BBURN becomes the first cultural deflation brand on Solana.",
              },
            ].map((phase, i) => (
              <div
                key={i}
                className="w-full overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-[#101c16] via-[#14251d] to-[#101c16] backdrop-blur-md p-8 md:p-10 hover:border-white/20 hover:bg-white/[0.055] transition-colors"
              >
                <div className="mb-3 flex items-center gap-3">
                  <span className="text-2xl">{phase.icon}</span>
                  <h4 className="text-xl md:text-2xl font-semibold text-amber-200">{phase.title}</h4>
                </div>
      ...
      <section id="wallets" className="scroll-mt-28 md:scroll-mt-32 lg:scroll-mt-36 mx-auto max-w-6xl px-4 py-10">
        ...
        <p className="text-white/60">The campfire burns in full view. Every wallet can be verified on Solana Explorer.</p>

        <div className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-3">
          <WalletCard title="Official Burn Wallet" address={BURN_WALLET} note="All $BBURN sent here are burned forever." />
          <WalletCard title="Treasury & Buybacks" address={TREASURY_WALLET} note="Funds for buybacks and operations." />
          <WalletCard title="Team & Marketing" address={MARKETING_WALLET} note="Supports growth, creators and partnerships." />
        </div>
      </section>

      {/* ===== Treasury Vault Lock ===== */}
      <section id="vault" className="scroll-mt-[88px] md:scroll-mt-[104px] lg:scroll-mt-[120px] mx-auto max-w-6xl px-4 mt-10">
        ...
        <p className="mt-3 text-sm text-white/55">Multisig escrow on Jupiter Lock. All details are verifiable on-chain.</p>
      </section>

      {/* ===== Contract Address ===== */}
      <section className="mx-auto max-w-5xl px-4 pt-16 pb-20 text-center">
        <h3 className="text-lg font-semibold text-amber-300 mb-3">Contract Address</h3>

        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3 rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 backdrop-blur-sm shadow-[0_0_20px_rgba(255,184,76,0.04)]">
          <code className="flex-1 font-mono text-[15px] text-white/90 text-left truncate select-text" title={FULL_TOKEN_ADDRESS}>
            {FULL_TOKEN_ADDRESS}
          </code>
          ...
          <div className="flex items-center gap-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-lg border border-white/10 bg-white/[0.06] px-3 py-1.5 text-sm font-medium text-white/80 hover:bg-white/[0.1] hover:text-white/90 transition"
            >
              View
            </a>
        ...
        <p className="mt-3 text-sm text-white/45">Always verify this address on Solana Explorer before interacting.</p>
      </section>
      ...
      <footer className="border-t border-white/10 bg-[#0d1a14] relative">
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 text-center text-sm text-white/60 space-y-4">
          ...
          <div className="flex justify-center gap-8 text-white/70 text-sm pt-2">
            <a href="https://x.com/burningbearcamp" target="_blank" rel="noopener noreferrer" className="hover:text-amber-300 transition" title="Join the X Community">
              🕊️ <span className="align-middle">X Community</span>
            </a>
            <a href="https://www.geckoterminal.com/solana/pools/9hVAstjuDaVGznN4iAUW6cb2nQxNieSW7vR2pu6x32CQ" target="_blank" rel="noopener noreferrer" className="hover:text-amber-300 transition" title="View on CoinGecko">
              🦎 <span className="align-middle">CoinGecko</span>
            </a>
            <a href="https://dexscreener.com/solana/9hvastjudavgznn4iauw6cb2nqxniesw7vr2pu6x32cq" target="_blank" rel="noopener noreferrer" className="hover:text-amber-300 transition" title="View on DexScreener">
      ...
      {!broadcast.on && (
        <a
          ...
          className="fixed z-50 right-6 bottom-[calc(1.25rem+env(safe-area-inset-bottom,0))] inline-flex items-center gap-2.5 rounded-full px-5 py-3 font-semibold text-[#120d05] bg-gradient-to-r from-amber-400 via-amber-300 to-yellow-400 ring-1 ring-amber-200/40 shadow-xl hover:scale-[1.04] hover:brightness-110 active:scale-[0.98] transition-transform duration-150"
          style={{
            boxShadow:
  ...
  return (
    <div>
      ...
      {variant === 'segments' ? (
        ...
        <div className="mt-1 text-3xl font-extrabold bg-gradient-to-r from-amber-200 via-amber-100 to-white bg-clip-text text-transparent md:text-[36px]" style={{ textShadow: '0 0 12px rgba(255,184,76,0.25)' }}>
          {value}
        </div>
      ) : (
        <div className="mt-1 text-3xl font-extrabold text-white/85 md:text-[36px]">{value}</div>
      )}
    </div>
  );
}
...
function LiveBug({ live, liveInMs, className = "" }: { live: boolean; liveInMs: number; className?: string }) {
  const fmt = (ms: number) => {
    const t = Math.max(0, Math.floor(ms / 1000));
    const h = Math.floor(t / 3600);
    const m = Math.floor((t % 3600) / 60);
...
function LowerThird({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div
      ...
      style={{ bottom: `calc(var(--safe-bottom, 0px) + ${TICKER_GAP}px)` }}
    >
...
function NowPlaying({ track, artist }: { track: string; artist?: string }) {
  return (
    <div
      ...
      style={{ top: `calc(var(--safe-top, 0px) + ${OVERLAY_TOP}px)` }}
    >
      <div className="flex items-center gap-2 rounded-full border border-amber-400/25 bg-black/60 backdrop-blur px-3.5 py-2 shadow-[0_8px_30px_rgba(0,0,0,.35)]">
        <span className="h-[10px] w-[10px] rounded-[3px] bg-amber-300 animate-pulse" />
        <span className="text-[13px] text-white/70">Now Playing</span>
        <span className="text-white/30">—</span>
        <span className="text-[15px] font-semibold text-amber-200">🐻 DJ Bear Live</span>
      </div>
    </div>
  );
}
...
function SegmentBox({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/[0.08] backdrop-blur px-3.5 py-2 text-[24px] md:text-[28px] font-extrabold tracking-tight leading-none text-white/90 shadow-[0_0_18px_rgba(0,0,0,0.30)]">
      {children}
    </span>
  );
}
...
function Colon({ soon = false }: { soon?: boolean }) {
  return (
    <span className="px-0.5 text-amber-200 colon-pulse colon-glow" style={{ ['--colon-speed' as any]: soon ? '1.1s' : '2.6s' }}>
      :
    </span>
  );
}
...
function Stat({ label, value, highlight = false }: { label: string; value: React.ReactNode; highlight?: boolean; }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0f1f19]/70 p-5 md:p-6 backdrop-blur">
      ...
      <div className={`mt-1 text-2xl font-extrabold ${highlight ? 'animate-pulse-fast text-amber-300 drop-shadow-[0_0_8px_rgba(255,184,76,0.45)]' : ''}`}>
        {value}
      </div>
    </div>
  );
...
function Pill({ children, className = '' }: { children: React.ReactNode; className?: string; }) {
  return (
    <span className={`inline-flex items-center rounded-2xl border border-white/15 bg-white/5 px-4 py-[7px] text-[15px] leading-none text-white/75 backdrop-blur ${className}`}>
      {children}
    </span>
  );
}
  ...
  return (
    <div className="md:hidden">
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label="Menu"
        className="ml-1 text-amber-200/80 text-[22px] hover:text-amber-100 hover:drop-shadow-[0_0_6px_rgba(255,200,100,0.6)] transition active:scale-95"
      >
        ☰
      </button>
...
function HalloweenBar() {
  return (
    <div className="z-20 w-full bg-gradient-to-r from-[#2b1a0f]/95 via-[#150e08]/90 to-[#2b1a0f]/95 border-b border-amber-500/30 backdrop-blur text-amber-100 flex items-center justify-center gap-3 py-3 shadow-[0_0_25px_rgba(255,140,0,0.25)]">
      <span className="flex items-center gap-3 text-[16px] sm:text-[17px] font-semibold">
        <span className="animate-pulse text-[18px]">🎃🎃🎃</span>
        <span className="text-amber-300 drop-shadow-[0_0_5px_rgba(255,200,0,0.3)]">Halloween Burn-a-thon</span>
...
function JupiterMark({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="0 0 32 32" className={className} role="img" aria-label="Jupiter">
      ...
      <path d="M7.5 17.25c5.8-1.95 10.9-1.95 16.7 0" stroke="#5b3a0a" strokeWidth="2.4" strokeLinecap="round" fill="none" />
...
/* =========================
   Broadcast: parses #broadcast?… and computes live/liveInMs
   Supports aliases:
   - revealAt / at  (ISO or ms)
   - revealIn / in  (ms)
========================= */
function useBroadcast(now: number) {
  const [on, setOn] = React.useState(false);
  const [params, setParams] = React.useState<URLSearchParams>(new URLSearchParams());
  const [live, setLive] = React.useState(true);
  const [liveInMs, setLiveInMs] = React.useState(0);
  ...
  React.useEffect(() => {
    const applySafeAreas = () => {
      ...
      const buyBtn = document.querySelector('a[aria-label="Buy $BBURN on Jupiter"]') as HTMLElement | null;
    ...
    const parse = () => {
      ...
      if (revealAt) {
        target = isFinite(Number(revealAt)) ? Number(revealAt) : Date.parse(revealAt);
      } else if (revealIn) {
        const delta = Number(revealIn);
        if (isFinite(delta)) target = Date.now() + delta;
      }
  ...
  // keep LIVE IN ticking
  React.useEffect(() => {
    if (!on) return;
    const atParam = params.get('revealAt') ?? params.get('at');
    const inParam = params.get('revealIn') ?? params.get('in');

    let target: number | undefined;
    if (atParam) target = isFinite(Number(atParam)) ? Number(atParam) : Date.parse(atParam);
    else if (inParam && isFinite(Number(inParam))) target = Date.now() + Number(inParam);
  ...
  return { on, params, live, liveInMs };
}

/* =========================
   BurnMoment overlay
========================= */
function BurnMoment({ show, onDone, sound, durationMs = 4500 }: { show: boolean; onDone?: () => void; sound?: string; durationMs?: number; }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] pointer-events-none bg-black/40 animate-[fadeIn_300ms_ease-out_forwards]" aria-hidden="true">
        ...
        {Array.from({ length: 36 }).map((_, i) => (
          <span
            ...
            style={{
              ...
              boxShadow: '0 0 6px rgba(255,180,80,.9), 0 0 12px rgba(255,160,60,.5)',
            }}
          />
        ))}
      </div>
      ...
      <style jsx>{`
        ...
        @keyframes rise { 0% { transform: translateY(0) translateX(0) scale(1); opacity: .9; }
                          70% { opacity: .9; }
                          100% { transform: translateY(-110vh) translateX(12px) scale(.6); opacity: 0; } }
        @keyframes glow { from { filter: blur(20px) brightness(1); } to { filter: blur(26px) brightness(1.25); } }
      `}</style>
    </div>
  );
}
